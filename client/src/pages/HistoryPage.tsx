  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-slate-600">Faça login para visualizar seu histórico</p>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-600 mt-2">
              {editals?.length || 0} edital(is) analisado(s)
            </p>
          </div>
          ...
          <Card className="border-slate-200">
            <CardContent className="pt-12 text-center">
              ...
              <p className="text-slate-600 mb-6">
                Comece fazendo upload de um edital para análise
              </p>
              <Button onClick={() => setLocation("/")} className="gap-2">
                <Plus className="w-4 h-4" />
                Analisar Edital
  ...
  return (
    <Card
      className={`border-2 transition-all hover:shadow-lg cursor-pointer ${
        hasCriticalDeadlines ? "border-red-200 bg-red-50" : "border-slate-200"
      }`}
      ...
      <CardContent className="pt-6">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-sm text-slate-600 mb-3">
              <span className="flex items-center gap-1">
                ...
                {formatDistanceToNow(new Date(edital.createdAt), {
                  addSuffix: true,
                  locale: ptBR,
                })}
              </span>
                ...
                {hasCriticalDeadlines && (
                  <Alert className="border-red-200 bg-red-50 py-2 px-3 flex-1 min-w-fit">
                    ...
                    <span className="text-xs font-semibold text-red-800">
                      {criticalCount} prazo(s) crítico(s)
                    </span>
                  </Alert>
                )}
                {highAlertCount > 0 && (
                  <Alert className="border-amber-200 bg-amber-50 py-2 px-3 flex-1 min-w-fit">
                    ...
                    <span className="text-xs font-semibold text-amber-800">
                      {highAlertCount} alerta(s) importante(s)
                    </span>
                  </Alert>
                )}
              </div>
          ...
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setLocation(`/analysis/${edital.id}`);
            }}
          >
